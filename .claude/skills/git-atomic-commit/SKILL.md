...
description: Analyze git status and create purpose-based atomic commits. Use when the user asks to commit changes, group changes into commits, generate commit messages, or preview a commit plan. Triggers include "커밋", "commit", "atomic commits", "commit plan", or any request to stage and commit current changes. Supports an optional scope path and a `--dry-run` preview mode. Groups files by logical purpose (not by directory or count) and matches the repo's detected commit style.
---
...
Load the **git-master** skill/agent and execute in COMMIT mode:
